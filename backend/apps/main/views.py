    ...
    return Response(
        {
            ...
            "pinned_posts_count": sum(
                1 for post in serializer.data if post.get("is_pinned", False)
            ),
        }
    )
...
def featured_posts(request):
    ...
    popular_posts = (
        Post.objects.with_subscription_info()
        .filter(status="published", created_at__gte=week_ago)
        .exclude(id__in=[post.id for post in pinned_posts])
