...
def cleanup_old_payments():
    ...
    old_payments = Payment.objects.filter(
        created_at__lt=cutoff_date, status__in=["failed", "cancelled"]
    )
...
def cleanup_old_webhook_events():
    ...
    old_events = WebhookEvent.objects.filter(
        created_at__lt=cutoff_date, status__in=["processed", "ignored"]
    )
...
def retry_failed_webhook_events():
    ...
    failed_events = WebhookEvent.objects.filter(
        status="failed", created_at__gte=retry_cutoff
    )[
        :50
    ]  # Ограничиваем количество для повторной обработки
