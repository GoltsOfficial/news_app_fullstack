...
class PaymentCreateSerializer(serializers.Serializer):
    ...
    payment_method = serializers.ChoiceField(
        choices=Payment.PAYMENT_METHOD_CHOICES, default="stripe"
    )
    ...
    def validate(self, attrs):
        ...
        if pending_payments:
            raise serializers.ValidationError(
                {
                    "non_field_errors": [
                        "User has pending payments. Please complete or cancel them first."
                    ]
                }
            )
...
class RefundSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ["id", "status", "created_by", "created_at", "processed_at"]
