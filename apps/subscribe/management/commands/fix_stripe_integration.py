    ...
    def handle(self, *args, **options):
        ...
        for plan in plans:
            ...
            if (
                ...
                self.stdout.write(
                    f"  ⏭️ План уже имеет реальный Stripe ID: {plan.stripe_price_id}"
                )
                continue
            ...
            try:
                ...
                price = stripe.Price.create(
                    ...
                    metadata={"plan_id": plan.id, "django_model": "SubscriptionPlan"},
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  ✅ План обновлен: {old_id} → {price.id}")
                )
